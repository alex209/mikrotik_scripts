...
:foreach iname,data in=$structVLAN do={
  if ($iname="MGMT") do={
    /interface bridge vlan add bridge=$brName vlan-ids=($data->"vid") tagged=$arrayTagInterface comment=($data->"comment");
    :set mgmtIntrface [/interface vlan add interface=$bridgeInterface vlan-id=($data->"vid") name=$iname comment=($data->"comment")];
    /interface bridge port set [find interface=[/interface ethernet get $lastEthInterface name]] pvid=($data->"vid") ingress-filtering=yes frame-types=admit-only-untagged-and-priority-tagged comment=($data->"comment");
  } else={
